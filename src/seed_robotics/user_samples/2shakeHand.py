...
ACCEPTABLE_HIGH = SENSOR_FORCE_TARGET + SENSOR_FORCE_TOLERANCE # = 250
ACCEPTABLE_LOW  = SENSOR_FORCE_TARGET - SENSOR_FORCE_TOLERANCE # = 150
...
# 1. neutral position
names_step_1            = ['r_w_rotation', 'r_w_adduction', 'r_w_flexion', 'r_th_adduction','r_ix_flexion','r_middle_flexion','r_ring_ltl_flexion'] # Name of the joints we want to move first
target_positions_step_1 = [2048,1000,2048,0,0,0,0,0] # Maximum position value : closed position; Minimum position value : open position
target_speeds_step_1    = [50, 50, 50, 50, 50, 50, 50, 50] # Speed 0 : Highest speed. Speed 50 : Low speed

# 2. 
names_step_2            = ['r_w_rotation', 'r_w_adduction', 'r_w_flexion', 'r_th_adduction','r_ix_flexion','r_middle_flexion','r_ring_ltl_flexion'] # Name of the joints we want to move first
target_positions_step_2 = [2048,1000,2048,0,2000,2000,2000,2000] # Maximum position value : closed position; Minimum position value : open position
target_speeds_step_2    = [50, 50, 50, 50, 50, 50, 50, 50] # Speed 0 : Highest speed. Speed 50 : Low speed
...
def computeGain(abs_val):
    gain = int(abs(abs_val - SENSOR_FORCE_TARGET)) # SENSOR_FORCE_TARGET = 200 # mN
    if gain > 200:
        return 200
    else:
        return gain
