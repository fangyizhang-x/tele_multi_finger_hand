...
def jointsCallback_original(joints_data):
...
#--------------------------------------------------------------
    if not outer:
        ...
        print('>>> Move to stand by position')
                ...
                for sensor in sensor_list:
                    ...
                    # Find its name
                        ...
                        if corresponding_joint:
                            ...
                            time.sleep(TIME_DELAY)
            outer = True
            print('>>> Exit Step3: adjustment.')
        
        if outer is True:
            print('>>> Shake hand: 1 second')
#--------------------------------------------------------------
            # Let armer shake hand.
            
#--------------------------------------------------------------
            print('>>> Move to grasp ball position')
            print('>>> Shake hand: 2 second')
            time.sleep(1)
            print('>>> Complete the handshake.')
            buildSpeedPosMsg(names_step_3,target_positions_step_3)
            time.sleep(1)
            buildSpeedPosMsg(names_step_3,target_positions_step_3)
